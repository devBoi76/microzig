...
pub fn GPIO(comptime port: u4, comptime num: u4, comptime mode: gpio.Mode) type {
    return switch (mode) {
        .input => struct {
            ...
            if (@field(config, port_field.name)) |port_config| {
                for (@typeInfo(Port.Configuration).Struct.fields) |field| {
                    if (@field(port_config, field.name)) |pin_config| {
                        ...
                        pin_field.type = GPIO(@intFromEnum(@field(Port, port_field.name)), @intFromEnum(@field(Pin, field.name)), pin_config.mode orelse .general_purpose_output);
                    ...
                    for (@typeInfo(Port.Configuration).Struct.fields) |field|
                        if (@field(port_config, field.name)) |pin_config| {
                            ...
                            switch (pin_config.get_mode()) {
                                ...
                                .general_purpose_output => output_gpios |= 1 << gpio_num,
                                else => {},
                            }
                        };
                }
